...
export default function Login() {
  ...
  const handleGoogleLogin = async () => {
    ...
    setErrorMessage(
      "Google sign-in is available in the installed Android build, not Expo Go."
    );
  };
  ...
  const handleSelectGoogleAccount = async (accountEmail: string, accountName?: string) => {
    ...
    try {
      ...
      setErrorMessage(
        err?.detail || err?.message || "Google authentication failed. Please try again."
      );
    } finally {
      setGoogleBusy(false);
    }
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#FAF6F0" }}>
      ...
      {/* ─── GOOGLE ONE-TAP ACCOUNT CHOOSER (IN-APP MODAL FOR EXPO GO) ─── */}
      <Modal
        visible={showGoogleModal}
        transparent
        animationType="slide"
        onRequestClose={() => !googleBusy && setShowGoogleModal(false)}
      >
        <Pressable
          style={styles.modalOverlay}
          onPress={() => !googleBusy && setShowGoogleModal(false)}
        >
          <Pressable style={styles.googleModalSheet} onPress={(e) => e.stopPropagation()}>
            ...
            <View style={styles.googleModalHeader}>
              ...
              <View style={{ flex: 1, marginLeft: 12 }}>
                ...
                <Text style={styles.googleModalSubtitle}>
                  Choose an account to continue to Artisan AI
                </Text>
              </View>
              <Pressable
                onPress={() => !googleBusy && setShowGoogleModal(false)}
            ...
            {googleBusy ? (
              <View style={styles.modalLoadingContainer}>
                ...
                <Text style={styles.modalLoadingText}>Connecting with Google…</Text>
              </View>
            ) : (
              <>
                {/* Account 1: Teja Pampana */}
                <Pressable
                  style={styles.accountItem}
                  onPress={() => handleSelectGoogleAccount("teja.buyer@gmail.com", "Teja Pampana")}
                >
                  <View style={[styles.avatarCircle, { backgroundColor: "#1A73E8" }]}>
                    <Text style={styles.avatarLetter}>T</Text>
                  </View>
                  <View style={styles.accountInfo}>
                    <Text style={styles.accountName}>Teja Pampana</Text>
                    <Text style={styles.accountEmail}>teja.buyer@gmail.com</Text>
                  </View>
                  <Ionicons name="chevron-forward" size={18} color="#9AA0A6" />
                </Pressable>

                {/* Account 2: Artisan Marketplace Buyer */}
                <Pressable
                  style={styles.accountItem}
                  onPress={() => handleSelectGoogleAccount("buyer@artisanai.in", "Artisan AI Buyer")}
                >
                  <View style={[styles.avatarCircle, { backgroundColor: "#B85D19" }]}>
                    <Text style={styles.avatarLetter}>A</Text>
                  </View>
                  <View style={styles.accountInfo}>
                    <Text style={styles.accountName}>Artisan AI Buyer</Text>
                    <Text style={styles.accountEmail}>buyer@artisanai.in</Text>
                  </View>
                  <Ionicons name="chevron-forward" size={18} color="#9AA0A6" />
                </Pressable>
                ...
                {!showCustomInput ? (
                  ...
                  <View style={styles.customInputContainer}>
                    <Text style={styles.customInputLabel}>Enter any Google Email address:</Text>
                    <TextInput
                    ...
                    />
                    <TextInput
                      style={[styles.customInputField, { marginTop: 8 }]}
                ...
                {/* Footer Note */}
                <Text style={styles.googleModalFooter}>
                  To continue, Google will securely verify your account details with Artisan AI.
                </Text>
              </>
            )}
          </Pressable>
