...
[![Test Suite](https://img.shields.io/badge/Tests-173%2F173%20Passing-brightgreen.svg)]()
...
| **Market Comparables Intelligence** | **Hardened SearXNG Market Research** extracting structured JSON-LD Schema.org (`Product`, `AggregateOffer`), OpenGraph, and HTML prices with private-IP SSRF defense, strict 4-level verified price invariants, attribute comparability matching, and offline-safe local prior fallback. | Real-time market anchoring without manual search friction, unverified claims, or LLM price hallucination. |
| **ML Demand Forecasting Engine** | **`scikit-learn` `RandomForestRegressor`** (`POST /api/ml/predict-demand`, `GET /api/ml/model-info`, `POST /api/ml/retrain`) predicting 7-day forward demand [0–100] using 13 structured economic and interaction features (`material_cost`, `labour_cost`, `packaging_cost`, `other_cost`, `total_cost`, `price_to_cost_ratio`, `stock`, `category_encoded`, `days_active`, `views`, `saves`, `enquiries`, `orders`). Validated with `TimeSeriesSplit(n_splits=3, gap=1)` temporal cross-validation with an explicit 7-day forecast gap. Features non-conversion cold-start neutral pricing (`1.000x`), evidence-based confidence rating, and admin-only retrain protection. | Grounded micro-forecasting with zero synthetic telemetry and transparent baseline provenance. |
...
7. **Production Secret Protection & Rotation**: Zero committed production secrets in version control; `.ebextensions/03_env.config` enforces `ENVIRONMENT=production` with runtime injection via AWS Systems Manager / Secrets Manager. Startup blocks all retired or insecure keys.
8. **Role-Based Telemetry Privacy**: `GET /api/events` strictly isolates telemetry by user role (Admins view global marketplace telemetry, Artisans query only their own product events, and Buyers/unprivileged accounts are blocked with `403 Forbidden`).
9. **Admin-Only ML Retraining Guard**: `POST /api/ml/retrain` requires `require_admin` authorization and enforces strict dataset volume thresholds ($\ge 20$ published items, $\ge 200$ buyer events, $\ge 14$ days span) after cold-start filtering.
...
### Test Coverage Summary (203/203 Passing):
- `test_ml_demand_hardening.py` (10 tests): 13-feature ML demand forecasting, cold-start non-conversion neutral pricing (`1.000x`), evidence-based confidence scoring, `TimeSeriesSplit(n_splits=3, gap=1)` temporal cross-validation with an explicit 7-day forecast gap, snapshot timestamp guards excluding ghost products created after $T$, admin-only retrain protection, and role-based telemetry isolation.
- `test_market_hardening.py` (15 tests): SSRF protection rejecting localhost/private IPs and bad schemes, safe redirect validation, Schema.org/OpenGraph/HTML price extraction, hard relevance filtering rejecting blogs/mismatches, verified price invariants, and independent SearXNG image/text search execution.
- `test_pricing_engine.py` (20 tests): Market median price adjustments, 20% cost floor preservation, upward/downward bounds, exact decimal arithmetic, auto-pricing with baseline anchors, and order event auto-pricing triggers.
- `test_v3_domain_isolation.py` (30 tests): Strict three-domain token isolation (Marketplace Buyer, Artisan Studio, Admin Console), cascade deletions with audit logging, draft defaults, Razorpay payment verification, and public product isolation.
- `test_pricing_matrix_contract.py` (9 tests): Exact boundary pricing cases, selling price schema verification, median weight configurations, and fallback cost-floor mechanics.
- `test_publish_security.py` (12 tests): Server-owned draft token provenance and anti-replay, heritage claims sanitization, PostgreSQL schema migration compatibility, and draft publish lifecycle authorization.
- `test_ondc_search.py`, `test_ondc_signing.py`, `test_ondc_eligibility.py` (21 tests): Beckn protocol v1.2 search, catalog query filtering, Ed25519 signing and verification, Blake-512 request hashing, timestamp expiry, and retail eligibility criteria.
- `test_market_research.py` (14 tests): SearXNG provider orchestration, Gemini search grounding verification with fake URL and fabricated price rejection, and rate limiting.
- `test_auth_security.py` (12 tests): Buyer and artisan authentication isolation, token version invalidation on password change, and Google OAuth payload rejection.
- `test_catalog_validator.py` (8 tests): Vision materials lock, unverified heritage claim rejection, and draft publishing validation.
- `test_p1_features.py`, `test_audit_improvements.py`, `test_p0_security_fixes.py`, `test_p0_p1_p2_architecture.py`, `test_production_database_foundation.py`, `test_trust_systems.py`, `test_auth_audit_fixes.py`, `test_image_enhancer.py`, `test_offline_audit_fixes.py`, `test_doc_spec_compliance.py`, `test_integration_phases.py`, `test_step1.py` - `test_step7.py`: Comprehensive coverage of health checks, catalog CRUD, multi-tenant security, idempotent offline queue, delivery notifications, and database constraints.
