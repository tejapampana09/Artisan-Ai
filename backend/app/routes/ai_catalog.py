...
class CatalogApproveRequest(BaseModel):
    artisan_facts: Optional[ArtisanFacts] = Field(default=None, description="Original canonical verified artisan facts object for validation")
...
async def process_voice_and_image(
    ...
    Orchestrates: ArtisanFacts 🔒 -> Gemini Catalog -> Validator -> Market Research -> Pricing Engine.
    """
    ...
    draft_payload = await process_full_catalog_pipeline(
    ...
    return AICatalogDraftResponse(**draft_payload)
...
def approve_and_publish_product(
    ...
    # 1. Final Price Safety Check against Minimum Fair Price floor
    ...
    if cost_basis > 0:
        margin_pct = req.min_margin_pct if req.min_margin_pct is not None else Decimal("0.20")
        minimum_fair_price = (cost_basis * (Decimal("1.0") + margin_pct)).quantize(Decimal("0.01"))
        if req.price < minimum_fair_price:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=f"Price ₹{float(req.price):,.2f} is below minimum fair price floor of ₹{float(minimum_fair_price):,.2f}. Please set price to at least ₹{float(minimum_fair_price):,.2f} to protect your minimum profit margin."
            )

    # 2. Final Validation on edited catalog content against ORIGINAL canonical facts 🔒
    final_materials = req.materials
    final_description = req.description
    final_craft_story = req.craft_story
    final_title = req.title

    if req.artisan_facts is not None:
        raw_edited = {
            "title": req.title,
        ...
        validated_edited = validate_catalog_draft(raw_edited, req.artisan_facts)
